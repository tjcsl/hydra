...
int hydra_read_SUBMIT(int fd,const void** exe_name_data,int *exe_name_len,uint16_t *slots) {
    int i; uint16_t u16; uint32_t u32;
...
int hydra_write_SUBMIT(int fd,const void*  exe_name_data,int exe_name_len,uint16_t  slots) {
    int i; uint16_t u16; uint32_t u32; char type;
    type = 5;
    if (write(fd, &type, 1) != 1) {return -1;}
